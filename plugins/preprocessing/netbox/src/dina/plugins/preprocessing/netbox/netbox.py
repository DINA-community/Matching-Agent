...
from dina.cachedb.model import Manufacturer, DeviceType, Device
...
class NetboxPreprocessor(PreprocessorPlugin):
    ...
    async def preprocess(self, data) -> List[Union[Manufacturer]]:
        ...
        for x in data[0]:
            ...
            if x.__class__.__name__ == "DeviceWithConfigContext":
                result.append(Device(nb_id=x.id,name=x.name,serial=x.serial,nb_devicetype_id=x.device_type.id))
        return result

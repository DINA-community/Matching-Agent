...
def _get_kwargs(
    *,
    ...
    if isinstance(body, list["ModuleBayTemplateRequest"]):
        _json_body = []
...
) -> Optional[list["ModuleBayTemplate"]]:
    if response.status_code == 200:
        response_200 = []
        _response_200 = response.json()
        for response_200_item_data in _response_200:
            response_200_item = ModuleBayTemplate.from_dict(response_200_item_data)
...
def _build_response(
    *, client: Union[AuthenticatedClient, Client], response: httpx.Response
) -> Response[list["ModuleBayTemplate"]]:
    return Response(

...
RUN pip install --find-links=/app/dist dina-matcher[isduba-fetcher,preprocessing-identity,preprocessing-default]
RUN rm -rf /app/dist
...
CMD ["csafsync", "--config", "/app/assets/csafsync.toml"]
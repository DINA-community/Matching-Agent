...
def _get_kwargs(
    *,
    ...
    if isinstance(body, list["CableTerminationRequest"]):
        _json_body = []
...
) -> Optional[list["CableTermination"]]:
    if response.status_code == 200:
        response_200 = []
        _response_200 = response.json()
        for response_200_item_data in _response_200:
            response_200_item = CableTermination.from_dict(response_200_item_data)
...
def _build_response(
    *, client: Union[AuthenticatedClient, Client], response: httpx.Response
) -> Response[list["CableTermination"]]:
    return Response(
...
def sync_detailed(
    *,
    ...
    Args:
        body (list['CableTerminationRequest']):
        body (list['CableTerminationRequest']):
